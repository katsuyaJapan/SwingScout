...
for x in universe:
  ...
  deviation_score=triangle(x['d25'],1.5,5.0,17.0); rsi_score=triangle(x['rsi'],51.0,17.0,15.0); volume_score=x['volumeScore']; price_hold_score=x['priceHoldScore']; close_location_score=x['closeLocationScore']
  ...
  individual=deviation_score+rsi_score+volume_score+price_hold_score+close_location_score+headroom_score+trend_score+risk_score+liquidity_score
  ...
  item={"code":x['code'],"name":x['name'],"sector":x['sector'],"sectorPhase":sec['phase'],"sectorScore":sec['score'],"close":x['close'],"score":round(score,3),"displayScore":round(score),"technical":round(individual,3),"liquidity":round(liquidity_score,3),"relativeVolume":x['relativeVolume'],"relativeVolumePrev":x['relativeVolumePrev'],"volumeSignal":x['volumeSignal'],"volumePhase":x['volumePhase'],"volumeSupplyDemand":x['volumeSupplyDemand'],"volumeSupplyDemandLabel":x['volumeSupplyDemandLabel'],"closeLocation":x['closeLocation'],"lowTrend3d":x['lowTrend3d'],"lowTrend3dSignal":x['lowTrend3dSignal'],"lowTrend3dLabel":x['lowTrend3dLabel'],"priceHoldSignal":x['priceHoldSignal'],"priceHoldLabel":x['priceHoldLabel'],"overheatSuppressed":x['overheatSuppressed'],"scoreBreakdown":{"sector":round(sec['score']*.30,2),"deviation":round(deviation_score,2),"rsi":round(rsi_score,2),"volume":round(volume_score,2),"priceHold":round(price_hold_score,2),"closeLocation":round(close_location_score,2),"headroom":round(headroom_score,2),"trend":round(trend_score,2),"risk":round(risk_score,2),"liquidity":round(liquidity_score,2),"penalty":round(penalty,2)},"setup":"初動候補" if x['volumePhase'] in ('DRY_UP_REACCELERATION','LOW_RISING_RECOVERY','BUYING_DEMAND','EARLY_ACCUMULATION') else "先回り候補","entry":f"{round(min(x['close'],x['ma5'])):,}円付近まで","invalidation":f"{round(x['low']):,}円割れ","reasons":reasons[:3],"riskFlags":earnings_flag,"earningsDate":earnings.get(x['code']),"earningsStatus":earnings_status.get(x['code'],'UNCONFIRMED'),"earningsPeriod":earnings_period.get(x['code']),"earningsDays":None,**rights.get(x['code'],{}),"metrics":{"closes":x['c'][-76:],"volumes":x['v'][-22:],"highs":x['h'][-22:],"lows":x['l'][-22:],"avgTurnover":x['at'],"activeTurnoverDays":x['activeDays']},"checks":{"price":True,"history":True,"liquidity":True,"fundamental":False,"earningsDate":x['code'] in earnings,"disclosure":False}}
